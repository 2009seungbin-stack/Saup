...
## First executed CI attempt and correction

Initial head `5cea743f7d4c1af6ff7d10f60dbe9347af8c0b3d`, normal CI `36213631920`: backend/frontend jobs succeeded. Separate acceptance `36213631799` built/started real containers but returned **8 passed / 1 failed**. The expanded suite opened more than ten fresh UI login sessions in a short minute window and correctly hit RATE_LIMITED. Restart steps were skipped, not passed. The test harness now paces fresh logins (at most eight per 61-second sliding window); production rate limits/auth code are unchanged. No HTTP login mocks or pre-inserted sessions were introduced.

The failing Playwright ARIA snapshot could include a disposable password. Test reporting now redacts the generated test secrets, and failed login handling clears the password input. The failed disposable Compose project was removed; this is not a claim that GitHub's old raw failure artifact was deleted. Do not redistribute that raw artifact as sanitized evidence. Subsequent source-matched checks must execute again.
