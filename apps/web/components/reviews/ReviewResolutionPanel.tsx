...
import MarketplaceCancellationPanel,{CUSTOMER_CATEGORY,MarketplaceReview} from './MarketplaceCancellationPanel';
...
type Review = Omit<MarketplaceReview,'snapshot'>&{id:string;category:string;entity_id:string;status:string;action:string|null;
...
export default function ReviewResolutionPanel({role,onSaved}:{role:string;onSaved:()=>Promise<void>}) {
  ...
  const correction=selected?.action==='CORRECT_UNCONFIRMED_PAYMENT_EVIDENCE';
  const customer=selected?.category===CUSTOMER_CATEGORY;
  async function reload(){if(!selected)return;setSelected(await api<Review>(`/v1/review-resolutions/${selected.id}`));await refresh();await onSaved();}
  return <section className="supplier-operations">
    <h2>검토 해결 · 증빙과 공급사 환급</h2>
    <p className="notice">검토 확인은 해결이 아닙니다. 이 화면은 송금하지 않으며, 공급사 환급을 기록해도 고객 환불·주문 종료는 별도의 마켓 취소 대사가 필요합니다.</p>
    ...
    {selected&&<section className="supplier-form"><h3>{selected.id}</h3><p role="status">검토 상태: {selected.status}</p>
      {customer&&<MarketplaceCancellationPanel key={`${selected.id}-${selected.stage}`} review={selected} role={role} onChanged={reload}/>}
      {!customer&&selected.blocked_reason&&<p className="notice">현재 실행 불가: {selected.blocked_reason}</p>}
      {!customer&&selected.snapshot&&<p>고정 지급 금액: {won(selected.snapshot.amount)} · 지급 ID: {selected.snapshot.payment_id}</p>}
      {!customer&&selected.eligible&&role!=='admin'&&<p>관리자만 해결 명령을 실행할 수 있습니다.</p>}
      {!customer&&selected.eligible&&role==='admin'&&<form onSubmit={submit}>
