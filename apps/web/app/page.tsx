...
export default function Dashboard() {
  ...
  return <main className="shell">
    ...
    <nav aria-label="운영 메뉴">{['상품','주문','검토 큐','결제','파일','연동'].map(t=><button key={t} className={tab===t?'selected':''} onClick={()=>setTab(t)}>{t}{t==='검토 큐'&&reviews.length>0&&<i>{reviews.length}</i>}</button>)}</nav>
