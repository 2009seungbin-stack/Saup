...
class AfterSales:
    ...
    def open_claim(self, order_id, external_id, category, amount, evidence):
        krw(amount)
        ...
        with self.factory.begin() as s:
            ...
            s.add(claim); s.flush()
            audit(s, "CLAIM_OPENED", claim.id, new={"category": category, "status": state}, correlation_id=order.correlation_id)
            if state == "READY": enqueue(s, "claim.submit", f"claim:{claim.id}", {"claim_id": claim.id})
    ...
    def supplier_response(self, claim_id, accepted_amount, accepted, actor="supplier"):
        krw(accepted_amount)
        with self.factory.begin() as s:
            lock_treasury(s); claim = s.get(Claim, claim_id)
            ...
            if claim.supplier_response:
                ...
                return
            if claim.status != "SUPPLIER_REVIEW": raise DomainError("CLAIM_NOT_SUBMITTED")
    ...
    def request_refund(self, claim_id, amount, key, actor=None):
        ...
        with self.factory.begin() as s:
            ...
            if prior:
                ...
                return prior.id
            claim = s.get(Claim, claim_id); order = s.get(Order, claim.order_id)
            ...
            if claim.status not in {"REFUND_ELIGIBLE", "MANUAL_REVIEW", "REFUNDED"}: raise DomainError("CLAIM_NOT_REFUNDABLE")
            total = s.scalar(select(func.coalesce(func.sum(Refund.amount), 0)).where(Refund.order_id == order.id,
                Refund.status.in_(["PENDING", "SENDING", "UNKNOWN", "SUCCEEDED", "MANUAL_APPROVAL"])))
            ...
            row = Refund(claim_id=claim.id, order_id=order.id, business_key=key, amount=amount,
                         status="MANUAL_APPROVAL" if manual else "PENDING")
            s.add(row); s.flush()
    ...
    def execute_refund(self, refund_id):
        with self.factory.begin() as s:
            ...
            if row.status in {"SUCCEEDED", "MANUAL_APPROVAL", "UNKNOWN"}: return
